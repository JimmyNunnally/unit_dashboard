...
#Calculate cofm for the AO:Done
#Add a notes collumn. Describe what units are attacking which, what was the roll 
...
#normal disto: 1st number is mean, second number is SD
roll=np.random.normal(50,10)
...
#Rule section
#Posture list and modifiers
'''
Attack_Posture_list=[Hasty_Defense,]
Defense_Posture_list=[Hasty_Defense,]
special_modifier=roll+input()
'''
#Def scenario():
...
def cofm_ao(df,turn):
    bf=df.loc[df['Color']=='BLUE']
    rf=df.loc[df['Color']=='RED']
    print('The current COFM is: ', bf['Firepower'].sum()/rf['Firepower'].sum())
